...
class DHScraper:
	...
	def get_section_from_article_url(self, url):
		match_dict = {
	...
	def write_day_articles(self, articles, date, dir_path):
	...
	def get_day_articles(self, url, dir_path):
		date, month = self.get_date_from_url(url)
		print('\n' + date + '-------------------\n\n')
		...
		for index, element in enumerate(elements):
			...
			if self.get_section_from_article_url(url) == '---':
				continue
			title, text, section = self.get_text_dh(url)
			if title == '':
				continue
			articles.append({'date':date, 'title':title, 'text':text, 'section':section, 'url':url})
			print(index, section, title)
		self.write_day_articles(articles, date, dir_path)
	...
	def write_date_range_articles(self, start_date, end_date, dir_path):
		...
		while cur_date <= end_date:
			...
			print("\n\n" + url)
			self.get_day_articles(url, dir_path)
			time.sleep(3)
